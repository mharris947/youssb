    ...
    <div className="min-h-screen bg-background pb-28">
      <header className="px-5 pt-12 pb-6 animate-fade-up">
        <div className="flex items-center gap-4">
          <Link href="/" className="p-2 -ml-2 rounded-full hover:bg-[#0c2074] transition-colors">
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 p-3 rounded-2xl min-w-[80px] transition-all ${
                  selectedContact === contact.id
                    ? 'bg-[#0c2074] text-[#0c2074]-foreground'
                    : 'bg-white hover:bg-[#0c2074]'
                }`}
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold ${
                    selectedContact === contact.id
                      ? 'bg-[#0c2074]-foreground/20'
                      : 'bg-[#223e99] text-[#0c2074]-foreground'
                  }`}
                >
                  {contact.avatar}
                </div>
          ...
          {others.map(contact => (
            <button
              ...
              className={`w-full flex items-center gap-4 p-4 transition-colors text-left ${
                selectedContact === contact.id ? 'bg-[#0c2074]/10' : 'hover:bg-[#0c2074]/50'
              }`}
            >
              <div className="w-12 h-12 rounded-full bg-[#223e99] flex items-center justify-center text-[#0c2074]-foreground font-bold">
                {contact.avatar}
              </div>
              ...
              {selectedContact === contact.id && (
                <div className="w-6 h-6 rounded-full bg-[#0c2074] flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-[#0c2074]-foreground" />
                </div>
              )}
            </button>
          ))}
      ...
      <section className="px-5 py-6 animate-fade-up stagger-5">
        <button
          ...
          className="w-full py-4 rounded-2xl bg-[#223e99] text-[#0c2074]-foreground font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
        >
          Send Money
        </button>
      </section>
