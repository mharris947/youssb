...
const spendingCategories = [
  { name: 'Food & Dining', amount: 842.5, percentage: 35, color: 'bg-[#0c2074]' },
  { name: 'Shopping', amount: 456.2, percentage: 19, color: 'bg-accent' },
  { name: 'Transportation', amount: 312.8, percentage: 13, color: 'bg-[#1fad53]' },
  { name: 'Entertainment', amount: 245.0, percentage: 10, color: 'bg-[#0c2074]/60' },
    ...
    <div className="min-h-screen bg-background pb-28">
      ...
      <section className="px-5 mb-6 animate-fade-up stagger-1">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="banking-card">
            <div className="flex items-center gap-2 mb-2">
              <div className="p-1.5 rounded-lg bg-[#0c2074]/10">
                <TrendingDown className="w-4 h-4 text-[#0c2074]" />
              </div>
      ...
      <section className="px-5 mb-6 animate-fade-up stagger-2">
        ...
        <div className="banking-card">
            ...
            {monthlyData.map((data, index) => (
              <div key={data.month} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full flex gap-0.5 h-28 items-end">
                  ...
                  />
                  <div
                    className="flex-1 bg-[#0c2074]/40 rounded-t"
                    style={{ height: `${(data.spending / maxBarValue) * 100}%` }}
                  />
                </div>
                <span
                  className={`text-xs ${index === monthlyData.length - 1 ? 'font-semibold text-[#0c2074]' : 'text-gray-500'}`}
                >
                  {data.month}
                </span>
              </div>
          ...
          <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-gray-200">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-[#0c2074]/40" />
          ...
          {spendingCategories.map(category => (
            <div key={category.name} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="h-1.5 bg-[#0c2074] rounded-full overflow-hidden">
                  <div
