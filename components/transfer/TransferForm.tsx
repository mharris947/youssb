...
const AmountInput = ({
  ...
  <section className="px-5 mb-6 animate-fade-up stagger-2">
    <div
      className={`banking-card text-center py-8 ${hasError ? 'border-2 border-destructive' : ''}`}
    >
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 p-4 rounded-2xl transition-all ${
            selectedAccountId === account.id
              ? 'bg-[#0c2074]/10 border-2 border-[#0c2074]'
              : 'bg-white shadow-card border-2 border-transparent'
          }`}
        >
          <div className="p-2.5 rounded-md bg-[#0c2074] text-white">
      ...
      <FormInput
        ...
        helperText={
          routingNumber && routingNumber.length < 9
            ? `${9 - routingNumber.length} digits remaining`
            : ''
        }
      />
