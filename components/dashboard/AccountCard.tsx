    ...
    <div className={`banking-card min-w-[280px] ${cardStyles} ${isPrimary ? 'shadow-none' : ''}`}>
      <div className="flex items-start justify-between mb-4">
        ...
        <button
          ...
          className={`p-1.5 cursor-pointer rounded-full text-white ${chevronBg} transition-colors`}
        >
      ...
      <div className="flex justify-between pt-3 border-t border-current/10">
        <div className="flex justify-between w-full">
          ...
          {isAvailableBalance && (
            <div className="">
              <p className={`text-xs ${labelStyles}`}>Available Balance</p>
              <p className="font-bold text-balance">
                {showBalance ? formatCurrency(isAvailableBalance) : '••••••'}
              </p>
            </div>
          )}
        </div>
